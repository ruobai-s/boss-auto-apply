    ...
    public JobRegisterResult register(JobRecordRequest request) {
        ...
        try (Connection connection = databaseService.open()) {
            ...
            String sql = "INSERT INTO job_record(source, source_job_id, company_name, company_introduction, job_name, "
                    + "job_description, city, salary, job_url, published_at, apply_status, outsourcing_excluded, "
                    + "outsourcing_manual_review, outsourcing_level, outsourcing_reason, outsourcing_keyword, first_seen_at, last_seen_at) "
                    + "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
                    + "ON CONFLICT(source, source_job_id) DO UPDATE SET company_name=excluded.company_name, "
                    + "company_introduction=excluded.company_introduction, job_name=excluded.job_name, "
                    + "job_description=excluded.job_description, city=excluded.city, salary=excluded.salary, "
                    + "job_url=excluded.job_url, published_at=excluded.published_at, "
            ...
            try (PreparedStatement statement = connection.prepareStatement(sql)) {
                ...
                statement.setInt(12, decision.excluded() ? 1 : 0);
                statement.setInt(13, decision.manualReview() ? 1 : 0);
                statement.setString(14, decision.level());
                statement.setString(15, decision.reason());
                statement.setString(16, decision.matchedKeyword());
